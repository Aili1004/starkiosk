package com.snepo.givv.cardbrowser.services.tokens
{
	...
	public class CardTransferToken extends EventDispatcher implements IToken
	{
		...
		public function CardTransferToken ( )
		{
		...
		public function start ( data : * = null ) : void
		{
			this._seedData = <request>
													...
													<card><id>{data.cardID}</id><number>{data.printerCardID}</number></card>
									 	 	 </request>;
			if ( CardPrintingService.LOG ) Logger.log ("CardTransferToken::start --> " + _seedData );
			...
			timeoutTimer.reset();
			timeoutTimer.start();
			Logger.beginTimer( "Host Communication: CARD TRANSFER" );
		...
		protected function onDataResponse ( evt : Event ) : void
		{
			Logger.endTimer("");
			timeoutTimer.reset();
			if ( CardPrintingService.LOG ) Logger.log ( "CardTransferToken::response --> " + evt.target.data );
			this._response = new XML ( evt.target.data );

			trace ( "CardTransfer response:\n" + this._response );
		...
		{
			if ( CardPrintingService.LOG ) Logger.log ("CardTransferToken::notifyComplete");
			dispatchEvent ( new TokenEvent ( TokenEvent.COMPLETE, response ) );
		}

		public function notifyError ( reason : String = "" ) : void
		{
			Logger.log ("CardTransferToken::notifyError - " + reason);
