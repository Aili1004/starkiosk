package com.snepo.givv.cardbrowser.services.tokens
{
	...
	public class ReviewCartToken extends EventDispatcher implements IToken
	{
		...
		public function ReviewCartToken ( )
		{
		...
		public function startEx ( data : * = null, subtractFees : Boolean = false) : void
		{
			if ( CardPrintingService.LOG ) Logger.log ("ReviewCartToken::start" );
			_seedData = <request><messageVersion>{MESSAGE_VERSION}</messageVersion><subtractFees>{subtractFees}</subtractFees><password>{Model.getInstance().getPassword()}</password>{data}</request>;;

			_request = new URLRequest ( CardPrintingService.PAYMENT_ENDPOINT + "/api/cart/review_cart.xml" );
			_request.method = URLRequestMethod.POST;
			_request.contentType = "text/xml";
			_request.data = _seedData;
			trace ( "Review request:\n" + this._request.data );
			...
			timeoutTimer.reset();
			timeoutTimer.start();
			Logger.beginTimer( "Host Communication: REVIEW" );
		...
		protected function onDataResponse ( evt : Event ) : void
		{
			Logger.endTimer("");
			timeoutTimer.reset();
			if ( CardPrintingService.LOG ) Logger.log ( "ReviewCartToken::response" );
		...
		protected function onTimeout ( evt : TimerEvent ) : void
		{
			...
			if ( CardPrintingService.LOG ) Logger.log ("ReviewCartToken::notifyTimeout");
			notifyError ('157');
		}
		...
		{
			if ( CardPrintingService.LOG ) Logger.log ("ReviewCartToken::notifyComplete");
			dispatchEvent ( new TokenEvent ( TokenEvent.COMPLETE, response ) );
		}

		public function notifyError ( reason : String = "" ) : void
		{
			if ( CardPrintingService.LOG ) Logger.log ("ReviewCartToken::notifyError");
